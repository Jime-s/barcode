...
export function BarcodeCarousel({ isCollapsed }: BarcodeCarouselProps) {
  ...
  // 解析当前页面路径，兼容默认语言和非默认语言
  const segments = pathname.split('/').filter(Boolean) // 过滤掉空字符串
  const currentCodeFormat =
    locale === defaultLocale ? segments[0] : segments[1]
